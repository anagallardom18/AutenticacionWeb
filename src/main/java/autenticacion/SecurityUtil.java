 ...
 * ADAPTACIÓN PARA EL PROYECTO: Ana María Gallardo Muñoz
 * CONTEXTO: Adaptación del módulo de seguridad para soportar la persistencia 
 * de credenciales en base de datos mediante claves maestras externas.
 */
public final class SecurityUtil {
    ...
    static {
        ...
        try {
            // Usamos un generador de números aleatorios seguro
            aux = SecureRandom.getInstance(RANDOM_ALGORITHM);
        } catch (final NoSuchAlgorithmException e) {
            logger.error("Error en el algoritmo de números aleatorios", e);
        }
        random = (aux != null) ? aux : new SecureRandom();

        try {
            keyFactory = SecretKeyFactory.getInstance(KEY_ALGORITHM);
        } catch (final NoSuchAlgorithmException e) {
            throw new ExceptionInInitializerError("No se pudo obtener la factoría de claves: " + e.getMessage());
        }

  
        Security.setProperty("crypto.policy", "unlimited");

        String llaveSistema = System.getenv("AES_KEY_APP");

        if (llaveSistema != null && llaveSistema.length() == 32) {
            // Si la llave existe y tiene 32 caracteres, la usamos
            eKey = new SecretKeySpec(llaveSistema.getBytes(StandardCharsets.UTF_8), "AES");
            logger.info("SEGURIDAD: Clave maestra cargada correctamente desde el sistema.");
        } else {
            // FALLO SEGURO: Si no está la llave, el programa se detiene por seguridad
            String error = "ERROR FATAL: No se encuentra la variable de entorno 'AES_KEY_APP' o no tiene 32 caracteres.";
            logger.error(error);
            // Lanzamos una excepción que impide que el servidor arranque con seguridad nula
            throw new RuntimeException(error);
        }

       
        associatedData = SecurityUtil.class.getPackageName().getBytes(StandardCharsets.UTF_8);
    }
